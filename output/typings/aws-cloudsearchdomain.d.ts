...
The options you can configure vary according to which parser you use:
 ...
 &amp;#42; defaultOperator : The default operator used to combine individual terms in
   the search string. For example: defaultOperator: &#x27;or&#x27; . For the dismax 
   parser, you specify a percentage that represents the percentage of terms in
   the search string (rounded down) that must match, rather than a default
   operator. A value of 0% is the equivalent to OR, and a value of 100% is
   equivalent to AND. The percentage must be specified as a value in the range
   0-100 followed by the percent (%) symbol. For example, defaultOperator: 50% .
   Valid values: and , or , a percentage in the range 0%-100% ( dismax ).
 ...
 * operators : An array of the operators or special characters you want to
   ...
   parentheses. Disabling near disables the ability to use the ~ operator to
   ...
   ability to use a backslash ( \ ) to escape special characters within the
   ...
   prevents Vietnamese words from being split incorrectly.) For example, you
   could disable all operators other than the phrase operator to support just
 ...
 * phraseFields : An array of the text or text-array fields you want to use for
   ...
   specify a field weight, append a caret ( ^ ) symbol and the weight to the
   field name. For example, to boost phrase matches in the title field over the 
 ...
 * structured : perform advanced searches by combining multiple expressions to
   ...
   boosting, matchall , and near . For more information, see Constructing
   Compound Queries
