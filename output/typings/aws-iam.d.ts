...
the Tools for Amazon Web Services [http://aws.amazon.com/tools/] page.We
recommend that you use the AWS SDKs to make programmatic API calls to IAM.
...
a role, use PutRolePolicy . For more information about policies, refer to 
...
in a role&#x27;s trust policy to establish a trust relationship between AWS and a
SAML identity provider. You can create an IAM role that supports Web-based
single sign-on (SSO) to the AWS Management Console or one that supports API
access to AWS.
...
[http://docs.aws.amazon.com/general/latest/gr/signature-version-4.html] .For
...
instance.For more information about instance profiles, go to About Instance Profiles
[http://docs.aws.amazon.com/IAM/latest/UserGuide/AboutInstanceProfiles.html] .
     *
     * @error NoSuchEntityException   
     * @error DeleteConflictException   
...
either make the access key inactive or delete it. For more information about
...
users, groups, and roles that it is attached to, and you must delete all of the
policy&#x27;s versions. The following steps describe the process for deleting a
managed policy: 1. Detach the policy from all users, groups, and roles that the
 ...
 2. Delete all versions of the policy using DeletePolicyVersion . To list the
    ...
    process.
 3. Delete the policy (this automatically deletes the policy&#x27;s
...
as a principal in their trust policies. Any attempt to assume a role that
references a SAML provider that has been deleted will fail.
...
policy, use the DeleteGroupPolicy API. For information about policies, refer to 
Managed Policies and Inline Policies
[http://docs.aws.amazon.com/IAM/latest/UserGuide/policies-managed-vs-inline.html] 
in the IAM User Guide .
     *
...
get the context keys from policies associated with an IAM user, group, or role,
use GetContextKeysForPrincipalPolicy .

Context keys are variables maintained by AWS and its services that provide
details about the context of an API query request, and can be evaluated by using
the Condition element of an IAM policy. Use GetContextKeysForCustomPolicy to
understand what key names and values you must supply when you call 
SimulateCustomPolicy . Note that all parameters are shown in unencoded form here
for clarity, but must be URL encoded to be included as a part of a real HTML
request.
     *
     * @error InvalidInputException   
     */
...
IAM user, group, or role. If you specify a user, then the request also includes
...
Context keys are variables maintained by AWS and its services that provide
details about the context of an API query request, and can be evaluated by using
the Condition element of an IAM policy. Use GetContextKeysForPrincipalPolicy to
understand what key names and values you must supply when you call 
...
[http://docs.aws.amazon.com/IAM/latest/UserGuide/AboutInstanceProfiles.html] .
For more information about ARNs, go to ARNs
...
is attached to. For a list of the specific users, groups, and roles that the
policy is attached to, use the ListEntitiesForPolicy API. This API returns
metadata about the policy. To retrieve the policy document for a specific
...
For more information about the types of policies, refer to Managed Policies and
Inline Policies
[http://docs.aws.amazon.com/IAM/latest/UserGuide/policies-managed-vs-inline.html] 
in the IAM User Guide .
     *
...
For more information about roles, go to Using Roles to Delegate Permissions and
Federate Identities
...
MFA devices. Assignment status can be Assigned , Unassigned , or Any .
...
A user can also have a managed policy attached to it. To attach a managed policy
...
running on the instance.For more information about roles, go to Working with Roles
[http://docs.aws.amazon.com/IAM/latest/UserGuide/WorkingWithRoles.html] . For
more information about instance profiles, go to About Instance Profiles
[http://docs.aws.amazon.com/IAM/latest/UserGuide/AboutInstanceProfiles.html] .
     *
...
in the IAM User Guide . To change a group name the requester must have
appropriate permissions on both the source object and the target object. For
example, to change Managers to MGRs, the entity making the request must have
permission on Managers and MGRs, or must have permission on all (&amp;#42;). For more
...
provider&#x27;s certificate and is validated by the thumbprint, it is a best practice
to limit access to the UpdateOpenIDConnectProviderThumbprint action to
...
in the IAM User Guide . To change a server certificate name the requester must
have appropriate permissions on both the source object and the target object.
For example, to change the name from ProductionCert to ProdCert, the entity
making the request must have permission on ProductionCert and ProdCert, or must
...
more information, see Renaming an IAM User
...
Inactive means it is not. **/
    ...
    export interface AddClientIDToOpenIDConnectProviderRequest {
        /** The Amazon Resource Name (ARN) of the IAM OpenID Connect (OIDC) provider to add
...
embedded in. **/
    ...
    export interface DeleteOpenIDConnectProviderRequest {
        /** The Amazon Resource Name (ARN) of the IAM OpenID Connect provider to delete. You
can get a list of OpenID Connect provider ARNs by using the 
ListOpenIDConnectProviders action. **/
        OpenIDConnectProviderArn: arnType;
    }
...
If the response includes any keys in this list, then the reported results might
be untrustworthy because the simulation could not completely evaluate all of the
conditions specified in the policies that would occur in a real world request. **/
...
elements. Each document is specified as a string containing the complete, valid
JSON text of an IAM policy. **/
...
to be included as a part of a real HTML request. **/
        ...
        /** A optional list of additional policies for which you want list of context keys
used in Condition elements. **/
        PolicyInputList?: SimulationPolicyListType;
    }
...
for the Marker parameter in a subsequent pagination request. **/
    ...
    export interface GetOpenIDConnectProviderRequest {
        /** The Amazon Resource Name (ARN) of the IAM OpenID Connect (OIDC) provider to get
information for. You can get a list of OIDC provider ARNs by using the 
ListOpenIDConnectProviders action. **/
        OpenIDConnectProviderArn: arnType;
    }
...
OpenID Connect provider. For more information, see CreateOpenIDConnectProvider . **/
        ThumbprintList?: thumbprintListType;
        /** The date and time when the IAM OpenID Connect provider entity was created in the
AWS account. **/
        CreateDate?: dateType;
    }
...
are attached to a user, group, or role. When OnlyAttached is false , or when the
...
original user name. **/
        UserName: existingUserNameType;
        /** New path for the user. Include this parameter only if you&#x27;re changing the user&#x27;s
path. **/
        NewPath?: pathType;
