...
declare module "aws-sdk" {
 ...
 might not behave as expected.
  ...
  export class StorageGateway extends Service {
    ...
    endpoint: Endpoint;
    /**
     * This operation activates the gateway you previously deployed on your host. For
more information, see Activate the AWS Storage Gateway
[http://docs.aws.amazon.com/storagegateway/latest/userguide/GettingStartedActivateGateway-common.html] 
. In the activation process, you specify information such as the region you want
to use for storing snapshots, the time zone for scheduled snapshots the gateway
...
volume architecture (see Storage Gateway Concepts
...
want to add cache, and one or more disk IDs that you want to configure as cache.
    ...
    /**
     * This operation adds one or more tags to the specified resource. You use tags to
add metadata to resources, which you can use to categorize these resources. For
example, you can categorize resources by purpose, owner, environment, or team.
Each tag consists of a key and a value, which you define. You can add tags to
...
using the CreateCachediSCSIVolume or CreateStorediSCSIVolume API. For
...
automatically initiated on the volume.
    ...
    /**
     * This operation returns the description of the gateway volumes specified in the
request. The list of gateway volumes in the request must be from one gateway. In
the response Amazon Storage Gateway returns volume information sorted by volume
ARNs.
     *
     * @error InvalidGatewayRequestException   
     * @error InternalServerError   
...
operation is supported only for the gateway-stored volume architecture. This
...
metadata or the disk content is corrupted).
    ...
    /**
     * This operation lists iSCSI initiators that are connected to a volume. You can
use this operation to determine whether a volume is being used or not.
     *
     * @error InvalidGatewayRequestException   
     * @error InternalServerError   
     */
...
storage. At this point you can reconfigure the disks as cache disks.
...
your applications can read from or write to the gateway&#x27;s storage volumes and
...
closed and initiators must reconnect with the new credentials.
    ...
    /**
     * This operation updates a gateway&#x27;s metadata, which includes the gateway&#x27;s name
and time zone. To specify which gateway to update, use the Amazon Resource Name
(ARN) of the gateway in your request.

For Gateways activated after September 02, 2015, the gateway&#x27;s ARN contains the
