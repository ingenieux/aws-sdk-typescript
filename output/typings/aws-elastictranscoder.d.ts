 ...
 *   MergeRetain: Elastic Transcoder transcodes both embedded and sidecar
   ...
   CaptionSources is empty, Elastic Transcoder omits all sidecar captions from
   the output files.
...
of each segment in seconds. For HLSv3 format playlists, each media segment is
 ...
 *   Permissions (Optional): The Permissions object specifies which users you
   want to have access to transcoded files and the type of access you want them
   to have. You can grant permissions to a maximum of 30 users and/or predefined
   Amazon S3 groups.
...
(Fragmented MP4) or ts (MPEG-TS), SegmentDuration is the target maximum duration
of each segment in seconds. For HLSv3 format playlists, each media segment is
stored in a separate .ts file. For HLSv4, MPEG-DASH, and Smooth playlists, all
...
little endian before inserting it into the PlayReady DRM headers. If you are
 ...
 *   Paused: The pipeline is not currently processing jobs. **/
    ...
    export interface VideoParameters {
        /** The video codec for the output file. Valid values include gif, H.264, mpeg2, vp8
, and vp9. You can only specify vp8 and vp9 when the container type is webm, gif 
when the container type is gif, and mpeg2 when the container type is mpg. **/
        Codec?: VideoCodec;
