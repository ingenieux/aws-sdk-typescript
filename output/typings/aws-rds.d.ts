...
You can copy an encrypted DB snapshot from another AWS Region. In that case, the
region where you call the CopyDBSnapshot action is the destination region for
 ...
 *   PreSignedUrl - A URL that contains a Signature Version 4 signed request for
   ...
   contains the encrypted DB snapshot to be copied.
    ...
    *   DestinationRegion - The AWS Region that the encrypted DB snapshot will be
      ...
      action in the us-east-1 region and provide a presigned URL that contains a
      call to the CopyDBSnapshot action in the us-west-2 region. For this
...
see Copying an Encrypted DB Snapshot to Another Region
...
region. KMS encryption keys are specific to the region that they are created in,
...
CopyDBSnapshot API action in the AWS region that contains the source DB snapshot
to copy. The PreSignedUrl parameter must be used when copying an encrypted DB
snapshot from another AWS region.

The presigned URL must be a valid request for the CopyDBSnapshot API action that
can be executed in the source region that contains the encrypted DB snapshot to
...
values:
 ...
 &amp;#42;   DestinationRegion - The AWS Region that the encrypted DB snapshot will be
   ...
   CopyDBSnapshot action in the us-west-2 region. For this example, the 
