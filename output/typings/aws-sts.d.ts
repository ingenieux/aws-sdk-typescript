...
declare module "aws-sdk" {
 ...
 information about activating a region for STS see Activating STS in a New Region
...
Important: You cannot call AssumeRole by using AWS account credentials; access
will be denied. You must use IAM user credentials or temporary security
...
[http://docs.aws.amazon.com/IAM/latest/UserGuide/roles-toplevel.html] in the 
Using IAM .
...
must also have a policy that allows you to call sts:AssumeRole .
...
AssumeRoleWithSAML , which can be up to 3600 seconds (1 hour) or until the time
...
minutes, even if values outside this range are specified.Optionally, you can
...
seconds (1 hour). By default, the temporary security credentials are valid for 1
hour.
...
security credentials of an AWS account (root), but this is not recommended.
...
to only the actions and resources they need access to. For more information, see 
IAM Best Practices
[http://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices.html] in the 
Using IAM .

The temporary security credentials that are obtained by using the long-term
credentials of an IAM user are valid for the specified duration, between 900
seconds (15 minutes) and 129600 seconds (36 hours). Temporary credentials that
are obtained by using AWS account (root) credentials have a maximum duration of
3600 seconds (1 hour)
...
a correct MFA code, then the API returns an access denied error.
...
IAM users are valid for the duration that you specify, between 900 seconds (15
minutes) and 129600 seconds (36 hours); credentials that are created by using
account credentials have a maximum duration of 3600 seconds (1 hour).
